...
static void Thread()
{
	...
	if (SHGetFolderPathA(NULL, CSIDL_PERSONAL, NULL, SHGFP_TYPE_CURRENT, file_name_and_path) == S_OK)
	{
		...
		{
			DWORD OldProtect = NULL, offsets[2] = { NULL }, samp = reinterpret_cast<DWORD>(GetModuleHandleA("samp.dll"));

			if (*reinterpret_cast<BYTE*>(samp + 0x129) == 0xF4) // 0.3.7-R1
			{
				offsets[0] = 0x270E88 + 0x4;
				offsets[1] = 0x309E05 + 0x4;
			}
			else if (*reinterpret_cast<BYTE*>(samp + 0x129) == 0xC) // 0.3.7-R2
			{
